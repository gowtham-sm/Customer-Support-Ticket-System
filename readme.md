...
\* \*\*UI Framework:\*\* MaterialDesignInXAML (for modern, edge-to-edge Material 3 aesthetics)
...
2\. Execute the provided `SupportTicketDB\_Script.sql` file to create the schema and populate the initial test data (including Admin and User accounts).

3\. Verify that the `users`, `tickets`, `ticketcomments`, and `ticketstatushistory` tables are created.
...
3\. Run the API project (IIS Express or Kestrel). 

4\. Note the localhost port it runs on (e.g., `https://localhost:7123`).
...
2\. Locate the `Session.cs` (or API Service class) and ensure the `BaseAddress` matches the port your Web API is running on.
...
\* \*\*Modern UX:\*\* Bypassed standard Win32 dialogs and basic styling in favor of asynchronous, non-blocking UI threads and a custom-built Material Design interface with floating labels and floating modal alerts.
